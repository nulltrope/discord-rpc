...
var (
	// timeout ensures we don't hang trying to connect to a non-existant socket
	defaultIPCSocketTimeout = time.Second * 2
	// discord can bind to multiple sockets, try from 0 up to this number
	defaultIPCSocketBindRange = 10
)

// DefaultIPC is a usable IPC transport with sensible defaults
var DefaultIPC *IPC = &IPC{
	BindRange:   defaultIPCSocketBindRange,
	BindTimeout: defaultIPCSocketTimeout,
}

// IPC is a transport which connects to the Discord client's local IPC socket
type IPC struct {
	Conn        net.Conn
	BindRange   int
